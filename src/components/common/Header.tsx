...
const Header: React.FC<HeaderProps> = ({ openApplyModal, openEligibilityModal, openPartnerModal }) => {
  ...
  return (
    <header className="fixed top-0 w-full bg-white border-b border-slate-200 z-50 shadow-sm transition-all font-sans">
      {/* 1. Top Auxiliary Bar (Desktop only) */}
      <div className="hidden lg:block bg-slate-900 text-slate-300 border-b border-slate-800 text-[11px] font-medium h-9">
        <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 h-full flex justify-end items-center gap-6">
          
          {/* Talk to Expert */}
          <div className="relative group h-full flex items-center cursor-pointer">
            <span className="flex items-center gap-1.5 hover:text-white transition-colors py-2">
              <span className="inline-block animate-bounce">📞</span> Talk to Expert <ChevronDown className="w-3 h-3 text-slate-500 group-hover:text-white" />
            </span>
            {/* Dropdown Menu */}
            <div className="absolute top-[80%] right-0 w-72 bg-white text-slate-800 rounded-xl shadow-2xl border border-slate-150 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-1 z-50 p-4 leading-relaxed">
              <h4 className="font-extrabold text-slate-900 text-sm mb-3 flex items-center gap-2 border-b border-slate-100 pb-2">
                <span>💬</span> Customer Support Desk
              </h4>
              <div className="space-y-3">
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-150">
                  <p className="text-[10px] uppercase tracking-wider text-slate-400 font-extrabold leading-none mb-1">Sales Enquiry</p>
                  <a href="tel:18005703888" className="text-sm font-black text-blue-600 hover:underline">1800 570 3888</a>
                </div>
                <div className="p-3 rounded-xl bg-slate-50 border border-slate-150">
                  <p className="text-[10px] uppercase tracking-wider text-slate-400 font-extrabold leading-none mb-1">Service Helpline</p>
                  <a href="tel:18002585616" className="text-sm font-black text-blue-600 hover:underline">1800 258 5616</a>
                </div>
              </div>
              <p className="text-[10px] text-slate-400 font-bold mt-3 text-center">
                Advisors available 7 days a week,<br/><strong>9:30 AM - 6:30 PM</strong>
              </p>
            </div>
          </div>
          ...
          <div className="relative group h-full flex items-center cursor-pointer">
            <span className="flex items-center gap-1.5 hover:text-white transition-colors py-2">
              <span>📱</span> Get The App <ChevronDown className="w-3 h-3 text-slate-500 group-hover:text-white" />
            </span>
            {/* Dropdown Menu */}
            <div className="absolute top-[80%] right-0 w-80 bg-white text-slate-800 rounded-2xl shadow-2xl border border-slate-150 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform translate-y-1 z-50 p-5">
              <div className="flex items-center gap-4">
                <div className="w-24 h-24 bg-slate-50 border border-slate-200 rounded-xl p-2 flex items-center justify-center flex-shrink-0">
                  {/* Clean SVG Placeholder QR Code representing the app download */}
                ...
                <div className="text-left leading-relaxed">
                  ...
                  <p className="text-[10px] text-slate-400 font-medium mb-3 leading-snug">
                    Scan the QR code to install the BanksCart application instantly.
                  </p>
                  <div className="flex gap-1.5">
                    <div className="px-2 py-1 bg-slate-900 text-white rounded-lg text-[8px] font-black text-center border border-slate-800 flex-1 hover:bg-slate-800">
                      Play Store
                    </div>
                    <div className="px-2 py-1 bg-slate-900 text-white rounded-lg text-[8px] font-black text-center border border-slate-800 flex-1 hover:bg-slate-800">
                      App Store
      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 h-16">
