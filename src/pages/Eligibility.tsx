...
import { CheckCircle, Loader2, AlertTriangle, User, Mail, Phone, Calendar, MapPin, Briefcase, Building } from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { initializeApp } from 'firebase/app';
import { getAuth, signInAnonymously, signInWithCustomToken, onAuthStateChanged } from 'firebase/auth';
import { getFirestore, collection, addDoc, serverTimestamp } from 'firebase/firestore';

// Expanded Mock MCA Company List (Same as in EligibilityCheckModal.tsx)
...
interface EligibilityPageProps {
  openApplyModal?: (loanType?: string) => void; // Passed from App.tsx if needed for buttons on this page
}

const Eligibility: React.FC<EligibilityPageProps> = ({ openApplyModal }) => {
  const [fullName, setFullName] = useState('');
  const [emailUsername, setEmailUsername] = useState('');
  const [emailDomain, setEmailDomain] = useState('gmail.com');
  const [phoneNumberDigits, setPhoneNumberDigits] = useState('');
  const [loanType, setLoanType] = useState(''); // No initialLoanType prop for a page
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-gray-50 flex items-center justify-center p-4 sm:p-6 font-inter">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg p-6 sm:p-8 relative transform transition-all scale-100 opacity-100">
        {!submissionSuccess ? (
          <>
            <h2 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-6 text-center">Check Your Loan Eligibility</h2>
